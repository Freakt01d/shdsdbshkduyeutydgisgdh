# Parquet Pipeline - GOLD ONLY (Async Pipelined, tuned for high row count)
#
# Gold characteristics (vs other 13 systems):
# - ~12M detail rows / ~66M audit rows per partition (~5x multiplier on JOIN)
# - Small rows: detail ~3KB, audit ~700 bytes
# - Net 65M output rows per day = ~450 parquet files per day
# - 34GB detail + 43GB audit = 77GB per day in DB
#
...
#   4. UPLOAD stage    - 3 parallel uploads (background, respects API pod limits)
#
# Restore overlaps with parquet: N+1 restores in DB while N is being parqueted.
# Max 2 partitions in DB at once (~154GB). Download keeps ahead of restore.
#
# OUTPUT FORMAT (unchanged):
# - Snappy parquet, 1000-row row groups, ~100MB files
# - Schema matches BlobFeeder exactly
# - Filename: data_gold_{date}_histmig_{host}_{guid}.snappy.parquet
# - Upload path: v2/gold/{month}/businessDay={biz_day}/
...
# DISK-CONSTRAINED: 200GB disk total, reserving ~20GB for OS.
# Working budget: ~180GB. Each gold partition = 77GB in DB.
# 2 partitions in DB at peak = 154GB + dumps + parquet staging = ~170GB. Fits.
MAX_DOWNLOADED_BUFFER = 3
# Max partitions in DB simultaneously. 2 = absolute max for 200GB disk.
MAX_RESTORED_BUFFER = 2
# How many parallel downloads within a day (detail + audit)
PARALLEL_DOWNLOADS_PER_DAY = 2
# Parquet workers. With MAX_RESTORED_BUFFER=2, more than 2 workers will sit idle.
# 2 workers race for the 2 partitions in DB - gives ~1.5-1.8x speedup vs serial.
PARALLEL_PARQUET = 2
# Parallel uploads. 2 parquet workers producing files concurrently.
PARALLEL_UPLOADS = 4
...
# ---- DB streaming (tuned for 2 parallel workers on gold) ----
# 50k rows * ~700 bytes avg = ~35MB per fetch, per worker.
# 2 workers = ~70MB concurrent cursor buffers. Safe.
DB_FETCH = 50000
# Flush to parquet every 20k rows. 20k * ~3KB = 60MB pyarrow table, per worker.
# 2 workers = ~120MB pyarrow memory concurrent. Fits comfortably.
ROWS_PER_WRITE = 20000
...
# ---- Disk safety ----
# With 200GB total, 2 partitions in DB = 154GB used. Need alert before disaster.
# If free drops below 30GB, something is leaking and we need to stop pulling new work.
MIN_DISK_FREE_GB = 30
# ================================
...
log_file = os.path.join(LOG_DIR, f"gold_async_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
logging.basicConfig(
    level=logging.INFO,
...
# ============ XML PARSING (optimized for gold's many-small-rows pattern) ============

# Precomputed singleton to avoid dict allocation on empty/no-flag rows
_NULL_FLAG = [{"Name": None, "Value": None, "FieldValue": None}]


def parse_flags(response_xml):
    # Fast path 1: empty/null response
    if not response_xml:
        return _NULL_FLAG

    # Fast path 2: substring check - if no "<Flag" in the string, skip full XML parse.
    # Substring check in Python is ~1000x faster than lxml parse.
    # Critical for gold where many audit rows have no flags.
    ...
    # Slow path: actual XML parse for rows that have flags
    try:
        ...
        return flags if flags else _NULL_FLAG
    except Exception:
        return _NULL_FLAG
...
def parquet_worker(worker_id, t_start):
    ...
    # Each worker pulls a partition from restored_q and processes it independently.
    # The first worker to see SHUTDOWN puts it back so others also exit.
    ...
    while True:
        ...
        # Overall progress (safe read under lock)
...
def main():
    ...
    log.info("=" * 60)
    log.info(f"  GOLD ASYNC PIPELINE | Host: {HOSTNAME}")
    ...
    # Spawn N parallel parquet workers - they all pull from restored_q
    pq_threads = []
    ...
    sf = os.path.join(LOG_DIR, f"gold_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
    with open(sf, "w") as f:
