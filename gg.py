...
# ============ CHECK EXISTING (fail-safe) ============

def has_existing_files(app, yyyymm, biz_day):
    """Check if target path already has files. Fail-safe: returns True on API error
    so we write to _new suffix rather than risk overwriting real data."""
...
def process_partition(month, day, upload_base):
    ...
    try:
        ...
        return file_count, row_count  # return partial progress
        ...
        # Try to remove the temp dir - only if empty
        try:
