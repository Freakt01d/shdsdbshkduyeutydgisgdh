...
audit_file = f”{sys_name}*raw_audit*{date_str}.dump”
...
fd = pool.submit(restore_one, detail_local, f”{sys_name}_{date_str}*detail”)
fa = pool.submit(restore_one, audit_local, f”{sys_name}*{date_str}_audit”)
...
guid = str(uuid.uuid4())
filename = f”data_{sys_name}*{date_str}*histmig*{HOSTNAME}*{guid}.snappy.parquet”
...
date_str = f”{month}{day:02d}”
detail_table = f”redservice.t_raw_detail_audit_{sys_name}*{month}*{date_str}”
audit_table = f”redservice.t_raw_audit_{sys_name}*{month}*{date_str}”
...
try:
    ...
    while True:
        ...
        for row in db_rows:
            ...
            if len(buf) >= ROW_GROUP_SIZE:
                batch = buf[:ROW_GROUP_SIZE]
                buf = buf[ROW_GROUP_SIZE:]

                if writer is None:
                    file_count += 1
                    writer, cur_file = make_writer(pq_dir, sys_name, date_str)

                writer.write_table(build_table(batch))
                row_count += len(batch)
    ...
    if buf:
        if writer is None:
            file_count += 1
            writer, cur_file = make_writer(pq_dir, sys_name, date_str)
        writer.write_table(build_table(buf))
        row_count += len(buf)
...
cur = conn.cursor()
cur.execute(f”TRUNCATE redservice.t_raw_detail_audit_{sys_name}*{month}*{date_str}”)
cur.execute(f”TRUNCATE redservice.t_raw_audit_{sys_name}*{month}*{date_str}”)
conn.commit()
cur.close()
conn.close()
except Exception as e:
