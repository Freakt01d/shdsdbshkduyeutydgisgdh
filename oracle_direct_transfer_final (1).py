...
def create_dest_connection():
    ...
    try:
        ...
        # Enable parallel DML since we're not using batcherrors
...
def direct_partition_transfer(args):
    ...
    try:
        ...
        # Can use PARALLEL hint now that batcherrors is removed
        insert_sql = f"""
            INSERT /*+ APPEND_VALUES PARALLEL(4) */ 
                ...
                while True:
                    try:
                        ...
                        for i in range(0, len(converted_rows), INSERT_BATCH_SIZE):
                            ...
                            # Bulk insert without batcherrors (not supported in all Oracle versions)
                            try:
                                ...
                                print(f"  Process {process_id}: Batch insert failed, trying row-by-row for {len(batch)} rows")
                                failed_count = 0
                                for row in batch:
                                    try:
                                        ...
                                        if failed_count <= 5:  # Only show first 5 errors
                                            print(f"    Process {process_id}: Row error: {str(row_error)[:100]}")
                                if failed_count > 0:
                                    print(f"    Process {process_id}: {failed_count} rows failed in this batch")
