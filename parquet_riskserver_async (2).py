...
ASYNC PIPELINE STAGES (all run concurrently, connected by queues):
  ...
  2. RESTORE stage   - 1 day restoring, 1 day queued (pre-restore next while processing)
  3. PARQUET stage   - 1 partition processing (streaming, serial)
  4. UPLOAD stage    - 4 parallel uploads (background)
...
- Filename: data_riskserver_{date}_histmig_{host}_{guid}.snappy.parquet
...
# Disk-constrained at 200GB. Each riskserver partition = 68GB in DB.
# 2 partitions max in DB = 136GB. Plus dumps + parquet = ~155GB. Fits.
MAX_DOWNLOADED_BUFFER = 5
MAX_RESTORED_BUFFER = 2

# Concurrency within stages - VM has 24 cores, 256GB RAM, lots of headroom
# How many parallel downloads within a day (detail + audit)
PARALLEL_DOWNLOADS_PER_DAY = 2
# Concurrent pg_restore workers - 2 days' restores can happen simultaneously
PARALLEL_RESTORE = 2
# Concurrent parquet workers - process both buffered partitions in parallel.
# Each worker uses ~2GB RAM (DB buffer + PyArrow). 2 workers = 4GB. Safe on 256GB.
# Each worker uses 1-2 cores. 2 workers = 2-4 cores of 24 available.
PARALLEL_PARQUET = 2
# Parallel uploads. With 2 parquet workers producing files, need more upload slots.
PARALLEL_UPLOADS = 4
...
# ---- DB streaming (tuned for 1MB rows, memory-aware) ----
# 100 rows * ~1MB = ~100MB per fetch round-trip. 
# Was 10 - too many round trips on a 311K row partition.
DB_FETCH = 100
# Flush parquet every 200 rows. 200 * 1MB = 200MB per flush batch.
# Buffer + PyArrow allocation = ~400MB peak. Avoids memory pressure.
# Was 1000 - that's 6GB per flush, can trigger swap on smaller VMs.
ROWS_PER_WRITE = 200
...
# Flags is a single nullable struct (NOT a list of structs)
FLAGS_TYPE = pa.struct([
...
def download_one(folder, filename, local_path):
    ...
    for attempt in range(DOWNLOAD_MAX_RETRIES):
        try:
            ...
            # Validate - pg_dump custom format starts with "PGDMP"
            # Also require min 10KB to reject empty/error responses
            sz = os.path.getsize(local_path)
            if sz < 10 * 1024:
                os.remove(local_path)
                log.warning(f"    DL {filename}: file too small ({sz} bytes), rejecting")
                return False
            try:
                ...
                if header != b"PGDMP":
                    os.remove(local_path)
                    log.warning(f"    DL {filename}: not a pg_dump file (header={header!r}), rejecting")
                    return False
            except Exception as e:
                log.warning(f"    DL {filename}: header check failed: {e}")
                if os.path.exists(local_path):
                    os.remove(local_path)
                return False
...
def download_day(month, day):
    ...
    audit_file = f"{SYSTEM}_raw_audit_{date_str}.dump"
...
def restore_day(date_str, detail_local, audit_local):
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        fd = pool.submit(restore_one, detail_local, f"{SYSTEM}_{date_str}_detail")
        fa = pool.submit(restore_one, audit_local, f"{SYSTEM}_{date_str}_audit")
        dok = fd.result()
        aok = fa.result()
...
# ============ XML PARSING (single-struct flags) ============

# BlobFeeder schema: Flags is a single nullable STRUCT, not a list.
# When response XML has 0 flags -> None (NULL in parquet)
# When response XML has 1+ flags -> first flag as a single struct
def parse_flags(response_xml):
    # Fast path 1: empty/null response -> NULL
    if not response_xml:
        return None

    # Fast path 2: substring check before lxml parse (1000x faster).
    # For riskserver's 1MB+ XML responses, skipping parse on flag-less rows is huge.
    if isinstance(response_xml, str):
        if "<Flag" not in response_xml:
            return None
        data = response_xml.encode()
    else:
        if b"<Flag" not in response_xml:
            return None
        data = response_xml

    # Slow path: parse XML and take FIRST flag
    try:
        root = etree.fromstring(data)
        first_flag = next(root.iter("Flag"), None)
        if first_flag is None:
            return None
        return {
            "Name": first_flag.get("Name", ""),
            "Value": first_flag.get("Value", ""),
            "FieldValue": None,
        }
    except Exception:
        return None
...
def make_writer(parquet_dir, date_str):
    guid = str(uuid.uuid4())
    filename = f"data_{SYSTEM}_{date_str}_histmig_{HOSTNAME}_{guid}.snappy.parquet"
...
def process_partition(month, day, upload_base):
    ...
    date_str = f"{month}{day:02d}"
    detail_table = f"redservice.t_raw_detail_audit_{SYSTEM}_{month}_{date_str}"
    audit_table = f"redservice.t_raw_audit_{SYSTEM}_{month}_{date_str}"
        ...
        while True:
            ...
            for row in db_rows:
                ...
                if len(buf) >= ROWS_PER_WRITE:
                    ...
                    # Progress log every 2 minutes - include memory state
                    now = time.time()
                    if now - last_log_time >= 120:
                        ...
                        try:
                            ...
                            mem_str = f"| Mem free: {mem.available/(1024**3):.1f}GB ({mem.percent:.0f}% used)"
                            proc = psutil.Process()
                            pmem = proc.memory_info().rss / (1024**3)
                            mem_str += f" | Py: {pmem:.1f}GB"
                        except Exception:
                            mem_str = ""
                        log.info(f"[PQ ] {date_str}: {row_count} rows | {rate:.0f} rows/sec | file {file_count} {mem_str}")
                        last_log_time = now
...
def truncate_tables(month, date_str):
    try:
        ...
        cur = conn.cursor()
        cur.execute(f"TRUNCATE redservice.t_raw_detail_audit_{SYSTEM}_{month}_{date_str}")
        cur.execute(f"TRUNCATE redservice.t_raw_audit_{SYSTEM}_{month}_{date_str}")
        conn.commit()
        cur.close()
        conn.close()
    except Exception as e:
...
def restore_worker(worker_id, t_start):
    # Stage 2: one of N parallel restore workers.
    # Each pulls a downloaded dump from downloaded_q, restores into PG, puts in restored_q.
    log.info(f"[RST#{worker_id}] worker started")
    while True:
        ...
        log.info(f"[RST#{worker_id}] starting {date_str}")

        ok = restore_day(date_str, detail_local, audit_local)
        if not ok:
            continue

        # Blocks if queue full (= MAX_RESTORED_BUFFER partitions already in DB)
        restored_q.put((month, day, date_str))
        log.info(f"[RST#{worker_id}] {date_str} -> parquet queue (qsize={restored_q.qsize()})")

    log.info(f"[RST#{worker_id}] worker done")


def parquet_worker(worker_id, t_start):
    # Stage 3: one of N parallel parquet workers.
    # Each pulls a partition from restored_q and processes it independently.
    # Multiple workers can hold separate PG connections and write to separate parquet files.
    log.info(f"[PQ#{worker_id}] worker started")
    while True:
        item = restored_q.get()
        if item is SHUTDOWN:
            # Put back for other workers to see
        ...
        if existing:
            ...
            log.info(f"[PQ#{worker_id}] {date_str}: existing files -> _new")
        else:
            upload_base = f"v2/{SYSTEM}/{month}/businessDay={biz_day}"

        t0 = time.time()
        log.info(f"[PQ#{worker_id}] starting {date_str}")
        ...
        if fc > 0 and rc > 0:
            ...
            rate = rc / max(dt, 1)
            log.info(f"[PQ#{worker_id}] DONE {date_str} | {fc} files | {rc} rows | {dt/60:.1f} min | {rate:.0f} rows/sec")
        else:
            with lock:
                results["failed_parquet"].append(f"{SYSTEM}_{date_str}")
            log.error(f"[PQ#{worker_id}] FAIL {date_str} (fc={fc}, rc={rc})")

        # Truncate AFTER parquet done - frees DB space for next pre-restored partition
        truncate_tables(month, date_str)
        log.info(f"[TRN#{worker_id}] {date_str} done")

        # Overall progress (read stats under lock)
    ...
    log.info(f"[PQ#{worker_id}] worker done")
...
def main():
    ...
    log.info(f"  Restore workers: {PARALLEL_RESTORE} parallel")
    log.info(f"  Parquet workers: {PARALLEL_PARQUET} parallel")
    log.info(f"  Uploads: {PARALLEL_UPLOADS} parallel, {UPLOAD_MAX_RETRIES} retries")
    ...
    # Spawn N parallel restore workers
    rst_threads = []
    for worker_id in range(PARALLEL_RESTORE):
        t = threading.Thread(target=restore_worker, args=(worker_id, t_start), name=f"restore-{worker_id}")
        rst_threads.append(t)

    # Spawn N parallel parquet workers
    pq_threads = []
    for worker_id in range(PARALLEL_PARQUET):
        t = threading.Thread(target=parquet_worker, args=(worker_id, t_start), name=f"parquet-{worker_id}")
        pq_threads.append(t)

    dl_thread.start()
    for t in rst_threads:
        t.start()
    for t in pq_threads:
        t.start()

    # Wait for all parquet workers to finish
    for t in pq_threads:
        t.join()
    log.info("[MAIN] all parquet workers done")

    # Wait for all restore workers
    for t in rst_threads:
        t.join()
    log.info("[MAIN] all restore workers done")
