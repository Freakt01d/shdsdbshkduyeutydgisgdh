# Parquet Pipeline - Historical Migration (14 systems, not riskserver)
...
# UPDATE THIS PER SERVER - riskserver runs in its own dedicated pipeline
SYSTEMS = [
    "gold", "eliot", "astro", "bga", "demeter", "efts",
...
# ============ UPLOAD (with retry + bumped timeouts) ============

def upload_file(local_path, remote_path):
    # Upload a parquet with retry. File kept on disk if all retries fail.
...
# ============ CHECK EXISTING (fail-safe, smart) ============

def has_existing_files(app, yyyymm, biz_day):
    # True if listing succeeded and found files.
    # False if listing succeeded with no files, or if path genuinely doesn't exist.
    # True (fail-safe) if the API is unreachable - forces _new suffix to protect data.
...
def process_one_partition(sys_name, month, day, upload_base):
    ...
    try:
        ...
        # Best-effort cleanup, never re-raise
        if writer is not None:
            try:
                writer.close()
            except Exception as e:
                log.warning(f"Process {sys_name}_{date_str} writer.close() in finally: {e}")
        if cursor is not None:
            try:
                cursor.close()
            except Exception as e:
                log.warning(f"Process {sys_name}_{date_str} cursor.close() in finally: {e}")
        if conn is not None:
            try:
                conn.rollback()
            except Exception:
            ...
            try:
                ...
                log.warning(f"Process {sys_name}_{date_str} conn.close() in finally: {e}")
        try:
            if os.path.isdir(pq_dir):
                ...
                if not remaining:
                    ...
                    log.info(f"Process {sys_name}_{date_str} pq_dir keeps {len(remaining)} file(s) (uploads pending/failed)")
        except Exception:
            pass
